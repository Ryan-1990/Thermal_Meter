...
COScopeCtrl::COScopeCtrl()
{
  ...
  m_crAlertColor_Low = RGB(240,88,41); 
  m_crAlertColor_High = RGB(255,238,98); 
...
COScopeCtrl::~COScopeCtrl()
{
  ...
  if (m_pbitmapOldAlert != NULL)
	  m_dcAlert.SelectObject(m_pbitmapOldAlert) ; 
...
/////////////////////////////////////////////////////////////////////////////
void COScopeCtrl::SetAlertColor(COLORREF color1, COLORREF color2)
{
	m_crAlertColor_Low = color1 ;
	m_crAlertColor_High = color2 ;

	m_penAlert_Low.DeleteObject() ;
	m_penAlert_Low.CreatePen(PS_SOLID, 0, m_crAlertColor_Low) ;
	m_penAlert_High.DeleteObject() ;
	m_penAlert_High.CreatePen(PS_SOLID, 0, m_crAlertColor_High) ;
...
void COScopeCtrl::InvalidateCtrl()
{
  ...
  // if we don't have one yet, set up a memory dc for the plot
  if (m_dcAlert.GetSafeHdc() == NULL)
  {
	  m_dcAlert.CreateCompatibleDC(&dc) ;
	  m_bitmapAlert.CreateCompatibleBitmap(&dc, m_nClientWidth, m_nClientHeight) ;
	  m_pbitmapOldAlert = m_dcAlert.SelectObject(&m_bitmapAlert) ;
  }
...
void COScopeCtrl::OnPaint() 
{
  ...
  if (memDC.GetSafeHdc() != NULL)
  {
	...
	memDC.BitBlt(0, 0, m_nClientWidth, m_nClientHeight, 
				 &m_dcAlert, 0, 0, SRCPAINT) ;  //SRCPAINT
...
/////////////////////////////////////////////////////////////////////////////
void COScopeCtrl::SetAlert(double dLower, double dUpper)
{
	//ASSERT(dUpper > dLower) ;

	double m_dLower;        // lower line
	double m_dUpper;        // upper line
	CPen *oldPen ;
	CRect rectCleanUp ;
	int High_Limit, Low_Limit;

	if(dLower < m_dLowerLimit) dLower = m_dLowerLimit;
	if(dUpper > m_dUpperLimit) dUpper = m_dUpperLimit;
	...
	if (m_dcAlert.GetSafeHdc() != NULL)
	{
		...
		m_dcAlert.BitBlt(m_rectPlot.left, m_rectPlot.top+1, 
		...
		m_dcAlert.FillRect(rectCleanUp, &m_brushBack) ;

		// draw the next line segement

		// grab the plotting pen
		oldPen = m_dcAlert.SelectObject(&m_penAlert_Low) ;
		
		High_Limit = m_rectPlot.bottom -(long)((m_dUpper - m_dLowerLimit) * m_dVerticalFactor) ;
		Low_Limit = m_rectPlot.bottom - (long)((m_dLower - m_dLowerLimit) * m_dVerticalFactor) ;

		m_dcAlert.MoveTo (m_rectPlot.left, Low_Limit) ;
		m_dcAlert.LineTo (m_rectPlot.right+1, Low_Limit) ;

		oldPen = m_dcAlert.SelectObject(&m_penAlert_High) ;
		m_dcAlert.MoveTo (m_rectPlot.left, High_Limit) ;
		m_dcAlert.LineTo (m_rectPlot.right+1, High_Limit) ;

		// restore the pen 
		m_dcAlert.SelectObject(oldPen) ;
	}
